...
async function getSteamData(request) {
  ...
  try {
    if (type === 'user') {
      ...
      if (info.achievements.every((a) => a.description && a.description.trim() !== '')) {
        return info;
      }

      let validXml = false;
      let xml;
      for (let id of steamUserIds) {
        userid = id;
      ...
      const allgood = list.every((a) => a.description && a.description.trim() !== '');

      //const url2 = `https://steamcommunity.com/stats/${appid}/achievements?l=${lang.api}`;  //this doesnt give hidden descriptions
      ...
      //const data = parser.parse(xml);
      //const achievements = data?.playerstats?.achievements?.achievement || [];
      list = Array.isArray(achievements) ? achievements : [achievements];
